    ...
    <div class="col-sm-12">
      ...
      <div class="ibox">
        ...
        <div class="ibox-content">
          <select class="form-control m-b" name="kaifatype" id="kaifatype">
          </select>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#pre").on("click",function(msg){
            if(pre>=1){
                query(pre,rows,name,type);
            }
        });
        $("#next").on("click",function(msg){
            if(next<=end){
                query(next,rows,name,type);
            }

        });
        $("#end").on("click",function(msg){
            query(end,rows,name,type);
        });
        //列表刷新
...
//                });
    ...
    function query(page,rows,name,type){
        $.ajax({
            type:"POST",
            url:"${pageContext.request.contextPath}/xuqiu/query",       //         &pageNumber=xx &name=xx &caid=xx
            data:"page="+page+"&rows="+rows+"&name="+name+"&type="+type,
            dataType:"json",
            success:function(msg){
                ...
                $.each(msg.rows,function(i,xuqiu){
                    ...
                    tr+="<tr> <td class='project-status'> <span class='label label-primary'>"+
                        ...
                        +"</small> </td> <td class='project-completion'> <small>当前进度："+xuqiu.test1+"%</small> <div class='progress progress-mini'> <div style='width: 28%;' class='progress-bar'></div> </div> </td><td class='project-actions'><button type='button' class='btn btn-white btn-sm' onclick=\"showXuqiuInfo('"+xuqiu.id+"')\"><i class='fa fa-folder'></i> 查看详情 </button></td></tr>";
                });
                $("#data").html(tr);
            }
    ...
    function queryByTj(){
        ...
        var kaifatype = $("#kaifatype").val();
        if(kaifatype!="z"&&kaifatype!=""&&kaifatype!=null){
            type=kaifatype;
        }
    ...
    function queryAllType(){
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/xuqiu/queryAllType",
            data:"",
            dataType:"json",
            success:function(msg){//msg----->List<Category>
                console.log(msg);
                var ops="<option value='z'>==请选择开发类型==</option>";
                $.each(msg.type,function(i,ca){
                    ops+="<option value='"+ca.bh+"'>"+ca.name+"</option>";
                });
                //查询的下拉菜单
                $("#kaifatype").html(ops);
            }
